...
MDSUncertainitySubWindow::MDSUncertainitySubWindow(QString mdsObjectName,
                                                   QWidget* parent)
    :QWidget(parent), mdsObjectName_(mdsObjectName),
      ui(new Ui::MDSUncertainitySubWindow)
{
    ...
    this->setWindowTitle("Uncertainity Space: "
                         + mdsObjectName);
    ...
    //this->parameterExploreModule = new MDSParameterExplorerTabWidget(this,this->mdsObjectName_);
...
MDSUncertainitySubWindow::~MDSUncertainitySubWindow()
{
    delete ui;
}

void MDSUncertainitySubWindow::setUpVtkRenderingWidget()
{
...
void MDSUncertainitySubWindow::updatePointLocations()
{
...
void MDSUncertainitySubWindow::setupClusterWidget()
{
...
void MDSUncertainitySubWindow::create_clusters_button_clicked()
{
    this->generateClusters();
}
void MDSUncertainitySubWindow::generateClusters()
{
    ...
    for (unsigned int i = 0; i < this->ui->numClusterSpinBox->value(); i++)
    {
        ...
        // the cluster on hand. TODO: find a way not to use nested for loop
...
QList<QStandardItem *> MDSUncertainitySubWindow::prepareRow(const QString &first,
...
void MDSUncertainitySubWindow::colorCluster(QModelIndex index)
{
    // Setup colors
    vtkSmartPointer<vtkUnsignedCharArray> colors =
            vtkSmartPointer<vtkUnsignedCharArray>::New();
...
void MDSUncertainitySubWindow::updateClusterView()
{
...
void MDSUncertainitySubWindow::highlightPoints(QStringList &selPoints)
{
    // Setup colors
