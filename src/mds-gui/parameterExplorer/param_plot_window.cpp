...
void param_plot_window::plot(QStringList &parameters,
                             VectorXf &senVector)
{
    // Create new VTK Widget
    qvtkWidget_ = new QVTKWidget(this);
    setWindowTitle(QString("Sensitivity Analysis"));
    ...
    // Create an array for listing parameter index
    arrId_ = vtkSmartPointer<vtkDoubleArray>::New();
    arrId_->SetName("Id");

    // Label for storing name of property
    labels_ = vtkSmartPointer<vtkStringArray>::New();
    labels_->SetName("Parameter");
    ...
    table_->AddColumn(labels_);
    table_->AddColumn(arrMetricValues_);
    table_->SetNumberOfRows(parameters.size());
    ...
    for (unsigned int i = 0; i < parameters.size(); ++i)
    {
        ...
        // Set Label
        table_->SetValue(i,1,parameters.at(i).toStdString().c_str());

        // Set Value
        table_->SetValue(i,2,senVector(i));
    }

    // Add horizontal bar plot, setting the colors etc
    vtkPlotBar *line = 0;
    line = vtkPlotBar::SafeDownCast(chart_->AddPlot(vtkChart::BAR));
    line->SetOrientation(1);
    line->SetInputData(table_, 0, 2);
    line->SetColor(67,	110	,238, 255);

    // Set up y - axis
    vtkAxis *x_axis = chart_->GetAxis(vtkAxis::LEFT);
    x_axis->SetBehavior(1);
    x_axis->SetMinimum(-1);
    ...
    vtkAxis *y_axis = chart_->GetAxis(vtkAxis::BOTTOM);
    y_axis->GetLabelProperties()->SetFontSize(14);
    y_axis->SetTitle("Standardizd Effect");
